...
/*
		...
		m_author = line.substr(line.find_first_not_of(' '), line.find(","));
		line.erase(0, line.find(",") + 1);

		m_title= line.substr(line.find_first_not_of(' '), line.find(",") - line.find_first_not_of(' '));
		line.erase(0, line.find(",") + 1);

		m_country = line.substr(line.find_first_not_of(' '), line.find(','));
		line.erase(0, line.find(',') + 1);

		m_price = std::stod(line.substr(line.find_first_not_of(' '), line.find(',')));
		line.erase(0, line.find(',') + 1);

		m_yearPublication = std::stoi(line.substr(line.find_first_not_of(' '), line.find(',')));
		line.erase(0, line.find(',') + 1);

		m_description = line.substr(0, '\n');
*/
