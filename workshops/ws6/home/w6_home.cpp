...
int main(int argc, char** argv)
{
	...
	{
		// Lambda expression that receives as parameter `const sdds::Vehicle*`
		//         and returns true if the vehicle has a top speed >300km/h
		auto fastVehicles = [] (const sdds::Vehicle* v) {return v->topSpeed() > 300 ? true : false; };
...

	{
		...
		auto brokenVehicles = [] (const sdds::Vehicle* v) {return (v->condition() == "broken")? true : false; };
